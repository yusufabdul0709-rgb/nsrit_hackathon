...
exports.createTicket = async (req, res) => {
  try {
    ...
    if (paymentMode === 'APSRTC_E_WALLET' || paymentMode === 'OFFLINE' || !paymentMode) {
      if (currentBalance < ticketFare) {
        return res.status(402).json({
          success: false,
          message: `Insufficient E-Wallet Balance. Ticket fare is Γé╣${ticketFare.toFixed(2)}, but available balance is Γé╣${currentBalance.toFixed(2)}.`,
          required: ticketFare,
      ...
      if (dbUser) {
        ...
        if (Transaction) {
          await Transaction.create({
            ...
            type: 'DEBIT_TICKET',
            title: `Bus Ticket - ${board} ΓåÆ ${dest}`,
            amount: ticketFare,
    ...
    return res.status(201).json({
      success: true,
      message: `Ticket issued! Γé╣${ticketFare.toFixed(2)} deducted from E-Wallet.`,
      ticket: ticketRecord,
    });
  } catch (err) {
