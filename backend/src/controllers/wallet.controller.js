...
// ΓöÇΓöÇΓöÇ GET WALLET BALANCE ΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ GET WALLET TRANSACTIONS ΓöÇΓöÇΓöÇ
  ...
  try {
    ...
    if (Transaction && isMongoConnected()) {
      ...
      const formatted = txs.map(t => ({
        ...
        amount: t.type === 'TOPUP' ? `+Γé╣${t.amount.toFixed(2)}` : `-Γé╣${t.amount.toFixed(2)}`,
    ...
    const formatted = userPayments.reverse().slice(0, 15).map(p => ({
      ...
      amount: p.amount > 0 ? `+Γé╣${Math.abs(p.amount).toFixed(2)}` : `-Γé╣${Math.abs(p.amount).toFixed(2)}`,
      positive: p.amount > 0,
...
// ΓöÇΓöÇΓöÇ ADD MONEY (GENERIC) ΓöÇΓöÇΓöÇ
const addMoney = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `Successfully added Γé╣${numAmount} to wallet`,
